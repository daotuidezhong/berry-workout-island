...
- 根因：常驻循环在 90—260ms 内直接混用站立行走、坐姿待机、舔爪、哈欠和起床素材，轮廓差异过大，造成视觉硬切。
- 九种常驻循环均改为单一姿势族，首尾的姿势、帧号和位移完全一致；状态总时长调整为 6.50—11.00 秒。
- 最短状态帧不低于 340ms；普通行走由 90ms/帧调整为 170ms/帧，起床由 280ms/帧调整为 420ms/帧，点击移动最短覆盖完整四帧步态。
- 位移过渡改为 320ms 缓入缓出；家具跳跃由 650ms 调整为 900ms，并继续只改变位置。
- 跨姿势族时使用同一图片节点的 260ms 轻淡入遮住轮廓硬切，不增加第二只猫、不缩放、不旋转。
- 浏览器连续采样三只猫的高活力高困倦循环，资源始终来自各自四帧行走序列，容器位置稳定且每帧只有一个图片节点；点击地面移动与睡眠打断后的完整起床序列均正常。
